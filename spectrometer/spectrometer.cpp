...
int GetDoubleFieldValue(double *val, const char *fieldname, const mxArray *prhs[]);
int GetStringFieldValue(char *val, const char *fieldname, const mxArray *prhs[]);
...
void DebugStringMessage(const char * msg)
{
#ifdef _DEBUG
    mexPrintf("AvaSpec debug message: %s\n", msg);
#endif
}
...
{
#ifdef _DEBUG
    mexPrintf("AvaSpec debug message: %d\n", intval);
#endif
}

void ErrorMessage(const char * msg)
{
    mexPrintf("AvaSpec ERROR message: %s\n", msg);
}
...
inline void ThrowString(const std::string& Error)
{
...
void CommandDispatch(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nlhs == 0) nlhs++;
    unsigned short wItems = sizeof (commands) / sizeof(CommandHandler);
    bool bFound = false;
    
    for (unsigned short wIndex = 0; wIndex < wItems && !bFound; wIndex++)
    {
...
void OnSetPwmOut(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    sprintf(msg, "port: %d, frequency: %lu, duty: %d", port, frequency, duty);
    DebugStringMessage(msg);
...
void OnMeasure(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    ready=0;
    timeout=(unsigned int) config.m_IntegrationTime*config.m_NrAverages+5000;
...
void OnSetSensitivityMode(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    unsigned long a_SensitivityMode;
    char msg[256];

    DebugStringMessage("OnSetSensitivityMode");
    // error checking for arguments
    if (nrhs != 1) 
        ThrowString ("wrong # of arguments - should be 'setsensitivitymode', mode ");
    a_SensitivityMode=(unsigned long) mxGetScalar(prhs[0]);

    sprintf(msg, "sensitivitymode: %lu", a_SensitivityMode);
    DebugStringMessage(msg);
